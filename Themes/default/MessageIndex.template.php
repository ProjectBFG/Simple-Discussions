...
function template_main()
{
	...
	<div class="pagination">
		', !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . '<a href="#bot" class="pull-left">' . $txt['go_down'] . '</a>' : '', '
	...
	echo '
		...
		if (!empty($context['topics']))
		{
			echo '
					...
					<th scope="col">', $context['topics_headers']['subject'], '</th>
					<th scope="col" class="center" width="14%">', $context['topics_headers']['starter'], '</th>
					<th scope="col" class="center" width="7%">', $context['topics_headers']['replies'], '</th>
					<th scope="col" class="center" width="7%">', $context['topics_headers']['views'], '</th>';
			// Show a "select all" box for quick moderation?
			if (empty($context['can_quick_mod']))
				echo '
					<th scope="col" width="22%">', $context['topics_headers']['last_post'], '</th>';
			else
				echo '
					<th scope="col" width="22%">', $context['topics_headers']['last_post'], '</th>';
			...
			if (!empty($context['can_quick_mod']) && $options['display_quick_mod'] == 1)
				echo '
					<th scope="col" width="24"><input type="checkbox" onclick="invertAll(this, this.form, \'topics[]\');" class="input_check" /></th>';
		...
		foreach ($context['topics'] as $topic)
		{
			...
			echo '
				<tr>
						...
						<div ', (!empty($topic['quick_mod']['modify']) ? 'id="topic_' . $topic['first_post']['id'] . '"  ondblclick="oQuickModifyTopic.modify_topic(\'' . $topic['id'] . '\', \'' . $topic['first_post']['id'] . '\');"' : ''), '>
							<div class="message_index_title">
								...
								<span class="preview', $topic['is_sticky'] ? ' bold_text' : '', '" title="', $topic[(empty($settings['message_index_preview_first']) ? 'last_post' : 'first_post')]['preview'], '">
									<span id="msg_', $topic['first_post']['id'], '">', $topic['first_post']['link'], ($context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '
									', $topic['new'] && $context['user']['is_logged'] ? '<a href="' . $topic['new_href'] . '" id="newicon' . $topic['first_post']['id'] . '"><span class="new_posts">' . $txt['new'] . '</span></a>' : '', '</span>
								</span>
							</div>
							<small id="pages', $topic['first_post']['id'], '">', $topic['pages'], '</small>
						</div>
					</td>
					<td class="', $color_class, ' center">
						', $topic['first_post']['member']['link'], '
					</td>
					<td class="', $alternate_class, ' center">
						', $topic['replies'], '
					</td>
					<td class="', $color_class, ' center">
						', $topic['views'], '
					...
					<td class="', $alternate_class, '">
						<span class="pull-right"><a href="', $topic['last_post']['href'], '"><img src="', $settings['images_url'], '/icons/last_post.png" alt="', $txt['last_post'], '" title="', $txt['last_post'], '" /></a></span>
